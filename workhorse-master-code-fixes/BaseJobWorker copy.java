...
public abstract class BaseJobWorker {
    ...
    //My fix for lawOfDemeter NEF
    public void onJobError(@Observes JobErrorEvent event) {
        // is that me?!
    	
    	//method chain without temp vars originally 
    	Job jobEvent = event.getJob(); //introduced temp var 
    	//jobEvent = event.getJob();
        if (getJobId() == jobEvent.getId()) {
    ...
    protected JobExecution create(Object parameters, Boolean priority, LocalDateTime maturity, Long batchId, Long chainId, Long chainPreviousExecutionId) {
    	Job jobVar = getJob(); //method chain fix by adding extra var
        Long jobId = jobVar.getId();
